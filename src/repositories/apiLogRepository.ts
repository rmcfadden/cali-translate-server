...
export const ApiLogRepository = {
  ...
  async getCountByInterval(apiKeyId: number, intervalUnit: string, interval: number): Promise<number>{
    const rows = await query<{count: number}>(`
      SELECT COALESCE(COUNT(id), 0) as count 
      FROM api_logs 
      WHERE api_key_id = ? 
      AND created >= DATE_SUB(UTC_TIMESTAMP(), INTERVAL ? ${intervalUnit.toUpperCase()})`, [apiKeyId, interval]);
    return rows[0].count;
  }
};